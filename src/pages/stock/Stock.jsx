...
export default function Stock() {
  ...
  // 🐛 BUG CORREGIDO: Calculamos las deudas automáticamente cuando users u orders cambian
  useEffect(() => {
  ...
  const handleBottleSubmit = async (e) => {
    ...
    if (success) {
      ...
      await loadBottleHistory(); // Esto actualiza el contexto, lo que disparará el useEffect de deudas
      addToast(`✅ Reporte de ${seller.name} guardado: ${reportedQuantity} botellones`, "success");
    }
  };

  const getMovementLabel = (mov) => {
    if (mov.movementType === "SALIDA_DETERIORO") return "⚠️ Deterioro";
    ...
    <div className="stock-page">
      ...
      <div className="stock-section">
        ...
        <div className="stock-tabs">
          <button className={`stock-tab ${activeTab === "entrada" ? "active" : ""}`} onClick={() => handleTabChange("entrada")}>➕ Agregar stock</button>
          <button className={`stock-tab ${activeTab === "salida" ? "active" : ""}`} onClick={() => handleTabChange("salida")}>➖ Quitar stock</button>
        </div>
        <div className="stock-inner-card">
          ...
          <form onSubmit={handleProductSubmit} className="stock-form">
            ...
            <button type="submit" className="stock-btn-primary full-width">{activeTab === "entrada" ? "🏭 Agregar stock" : "➖ Quitar stock"}</button>
          </form>
        </div>

        <h3>📦 Inventario actual</h3>
        {products.length === 0 ? (
          <EmptyState icon="📦" title="Sin productos" description="No hay productos registrados en el sistema." />
        ) : (
      ...
      <div className="stock-section bottles-section">
        <h2>🫙 Control de Botellones Vacíos</h2>
        <div className="stock-inner-card">
        ...
        {debts.length === 0 ? (
          <EmptyState icon="🫙" title="Sin deudas" description="No hay registros de botellones adeudados." />
        ) : (
          <div className="stock-grid">
            {debts.map((debt) => (
              <div key={debt.sellerId} className="debt-card">
                ...
                <p className={`debt-status ${debt.debt > 0 ? "debt-pending" : "debt-clear"}`}>
                  {debt.debt > 0 ? `⚠️ Deuda: ${debt.debt}` : "✅ Sin deuda"}
                </p>
              </div>
            ))}
          </div>
        ...
        {showBottleHistory && (
          <div style={{ marginTop: "20px" }}>
            ...
            {!loadingHistory && bottleHistory.length === 0 && (<EmptyState icon="📜" title="Sin historial" description="Aún no se han reportado botellones." />)}
      ...
      <div className="stock-section history-section">
        ...
        <div className="stock-filters">
          ...
          <button onClick={loadMovements} className="stock-btn-secondary">🔄 Actualizar</button>
        </div>

        {loadingMovements && <p style={{ color: "#64748b" }}>Cargando movimientos...</p>}
        {!loadingMovements && movements.length === 0 && (<EmptyState icon="📜" title="Sin movimientos" description="No se han registrado entradas ni salidas." />)}
